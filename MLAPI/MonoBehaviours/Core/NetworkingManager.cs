...
namespace MLAPI
{
    ...
    public class NetworkingManager : MonoBehaviour
    {
        ...
        /// <summary>
        /// Add's a passed NetworkedPrefab to the NetworkedPrefabs list.
        /// This allows for mods to add NetworkedPrefabs during runtime.
        /// Use AddNetworkedPrefabs to add entire arrays more efficiently.
        /// </summary>
        /// <param name="prefab"></param>
        public void AddNetworkedPrefab(NetworkedPrefab prefab)
        {
            if (
                NetworkConfig.NetworkedPrefabs.Contains(prefab) ||
                NetworkConfig.NetworkPrefabNames.ContainsValue(prefab.name))
            {
                Debug.LogError($"Attempting to add duplicate's of {prefab.name} to NetworkedPrefabs list");
                return;
            }
            //We don't want to 'OrderBy' as in the 'SetupNetPrefabs' method as that could screw up all cached indexes
            NetworkConfig.NetworkedPrefabs.Add(prefab);
            NetworkConfig.NetworkPrefabIds.Add(prefab.name, NetworkConfig.NetworkedPrefabs.Count - 1);
            NetworkConfig.NetworkPrefabNames.Add(NetworkConfig.NetworkedPrefabs.Count - 1, prefab.name);
        }

        /// <summary>
        /// Add's a passed NetworkedPrefab array to the NetworkedPrefabs list.
        /// This allows for mods to add NetworkedPrefabs during runtime.
        /// Use AddNetworkedPrefab to add individual elements
        /// </summary>
        /// <param name="prefabs"></param>
        public void AddNetworkedPrefabs(NetworkedPrefab[] prefabs)
        {
            int oldLast = NetworkConfig.NetworkedPrefabs.Count - 1;

            for (int i = 0; i < prefabs.Length; i++)
            {
                if (
                    NetworkConfig.NetworkedPrefabs.Contains(prefabs[i]) ||
                    NetworkConfig.NetworkPrefabNames.ContainsValue(prefabs[i].name))
                {
                    Debug.LogError($"Attempting to add duplicate's of {prefabs[i].name} to NetworkedPrefabs list");
                    return;
                }
            }

            NetworkConfig.NetworkedPrefabs.AddRange(prefabs); //It's better to add them all in one go, rather than one at a time
            /* I know I could move this into the above for-loop, it's just that if the above for loop were to
             * stop at any point, it would leave unused prefab names */
            for (int i = 0; i < prefabs.Length; i++)
            {
                NetworkConfig.NetworkPrefabIds.Add(prefabs[i].name, oldLast + i);
                NetworkConfig.NetworkPrefabNames.Add(oldLast + i, prefabs[i].name);
            }
        }
